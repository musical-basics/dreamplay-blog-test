...
/**
 * V2 Blog Generation API — SSE Endpoint
 * Same pattern as email generate route with blog-specific fields.
 */
...
async function urlToBase64(url: string) {
    try {
        ...
        return { base64: buffer.toString("base64"), mediaType: response.headers.get("content-type") || "image/jpeg" }
    } catch {
        return null
    }
}
...
export async function POST(req: Request) {
    try {
        ...
        const initialState: Partial<BlogState> = {
        ...
        const threadId = thread_id || `blog_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`
